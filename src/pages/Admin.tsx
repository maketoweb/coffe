...
import {
  ...
  Search, CheckCircle, AlertTriangle, Send, Bell, Ticket,
  Receipt, Printer, Check, X, MessageSquare, MessageCircle, ExternalLink, Upload, DollarSign, Package, ShoppingCart, User, Download, FileSpreadsheet, Eye, EyeOff, Calendar, RefreshCcw,
...
export const Admin: React.FC<AdminProps> = () => {
  const { 
    ...
    updateOrderStatus, updateOrderItems, addNotification, deleteNotification,
  ...
  const [, setFormNombre] = useState('');
  const [, setFormDescripcion] = useState('');
  const [, setFormCategoria] = useState('Hamburguesas');
  const [, setFormPrecio] = useState(0.00);
  const [, setFormStock] = useState(1);
  const [, setFormImages] = useState<string[]>([]);
  const [, setFormPromo] = useState(false);
  const [, setFormNuevo] = useState(false);
  const [, setFormVendido] = useState(false);
  const [, setFormDisponibilidad] = useState<'Disponible' | 'Agotado' | 'En Reposición'>('Disponible');
  ...
  const handleCreateBroadcast = async (e: React.FormEvent) => {
    ...
    console.warn('📢 Admin: Iniciando difusión de notificación:', broadcastTitle);
    ...
    if (adminSection === 'reports' || adminSection === 'settings') {
      ...
      if (!lastBackup || (now - Number(lastBackup)) > fifteenDays) {
        if (confirm("🗓️ Han pasado 15 días desde su último respaldo. ¿Desea descargar una copia de seguridad de sus datos ahora?")) {
          ...
          console.warn(`🍔 ${config.site_nombre || 'Tienda'}: Respaldo manual solicitado por periodo quincenal.`);
        }
      }
    }
  ...
  }, [foodItems, pickerSearch, searchItems]);
      ...
      {adminSection === 'settings' && (
        <div className="flex flex-col gap-6 animate-fade-in">
          ...
          <div className="flex flex-col gap-4 p-5 border border-violet-200 rounded-2xl bg-violet-50 shadow-sm">
            ...
            <button 
              onClick={async () => {
                console.warn('🧪 Iniciando prueba de comunicación Push...');
                ...
                if (success) {
                  console.warn('✅ Prueba enviada con éxito.');
